...
class LoginModel
{
    ...
    public function login()
    {
        ...
        if (password_verify($_POST['user_password'], $result->user_password_hash)) {
            ...
            Session::set('user_district', $result->user_district);
            Session::set('user_state', $result->user_state);
    ...
    public function loginWithCookie()
    {
        ...
        $query = $this->db->prepare("SELECT user_id, user_name, user_email, user_password_hash, user_active,
                                          user_account_type,user_level, user_state, user_district, user_has_avatar, user_failed_logins, user_last_failed_login
                                     FROM users
    ...
    public function editInterests_add($interestID,$remove){
        
        if($interestID!=0){
            $query = $this->db->prepare("SELECT ID FROM user_issues WHERE userID = :userID AND issueID = :issueID");
            $query->execute(array(':userID' => $_SESSION["user_id"], ':issueID' => $interestID));
            $count =  $query->rowCount();
            if (!$remove && $count < 1) {
                $query = $this->db->prepare("INSERT INTO user_issues (userID, issueID) VALUES (:userID, :issueID)");
                $query->execute(array(':userID' => $_SESSION["user_id"], ':issueID' => $interestID));
            ...
            }elseif($remove && $count >= 1){
                $query = $this->db->prepare("DELETE FROM user_issues WHERE userID = :userID AND issueID = :issueID");
                $query->execute(array(':userID' => $_SESSION["user_id"], ':issueID' => $interestID));
                $count = $query->rowCount();
        ...
        $sql = "SELECT u.issueID, i.issueName 
                FROM user_issues u
                RIGHT JOIN issuesList i 
                ON i.ID = u.issueID
                WHERE userID=:userID
                ";
        ...
        $jsonRet = json_encode($query->fetchAll());
    ...
    public function registerNewUser()
    {
        ...
        /*if (!$this->checkCaptcha()) {
            ...
            $_SESSION["feedback_negative"][] = FEEDBACK_ADDRESS_FIELD_EMPTY;
        } elseif (empty($_POST['user_state'])){
            $_SESSION["feedback_negative"][] = FEEDBACK_STATE_FIELD_EMPTY;
        } elseif (empty($_POST['user_zipcode'])){
            $_SESSION["feedback_negative"][] = FEEDBACK_ZIPCODE_FIELD_EMPTY;
            ...
            //Plug their address into googlemaps and get the LAT/LONG
                $address = $_POST["user_address"]."+".$_POST["user_city"]."+".$_POST["user_state"]."+".$_POST["user_zipcode"];
                ...
                if($json->{'status'}=="OK"){
                    ...
                    $districtInfo = new DistrictInfo($lat,$long);
                }else{
                    $_SESSION["feedback_negative"][] = FEEDBACK_ADDRESS_FIELD_INVALID;
                    return false;
                }
            
            
            // write new users data into database
            $sql = "INSERT INTO users (user_name, user_password_hash, user_email, user_state, user_district, user_creation_timestamp, user_activation_hash, user_provider_type)
                    VALUES (:user_name, :user_password_hash, :user_email, :user_state, :user_district, :user_creation_timestamp, :user_activation_hash, :user_provider_type)";
            $query = $this->db->prepare($sql);
            $query->execute(array(':user_name' => $user_name,
                                  ':user_password_hash' => $user_password_hash,
                                  ':user_email' => $user_email,
                                  ':user_state' => $districtInfo->state,
                                  ':user_district' => $districtInfo->district,
    ...
    public function _addUserToFollow($userID,$remove)
    {
        $error = FALSE;
        $query = $this->db->prepare("SELECT ID FROM user_follow WHERE userID = :current_userID AND followed_user_ID = :followed_user_ID");
        $query->execute(array(':current_userID' => $_SESSION["user_id"], ':followed_user_ID' => $userID));
        $count =  $query->rowCount();
        if (!$remove && $count < 1) {
            $query = $this->db->prepare("INSERT INTO user_follow (userID, followed_user_ID) VALUES (:current_userID, :followed_user_ID)");
            $query->execute(array(':current_userID' => $_SESSION["user_id"], ':followed_user_ID' => $userID));
            ...
            $query = $this->db->prepare("DELETE FROM user_follow WHERE userID = :current_userID AND followed_user_ID = :followed_user_ID");
            $query->execute(array(':current_userID' => $_SESSION["user_id"], ':followed_user_ID' => $userID));
            $count = $query->rowCount();
