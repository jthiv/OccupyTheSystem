...
class OverviewModel
{
    ...
    public function getUserProfile($user_id)
    {
        ...
        if(Session::get('user_logged_in')){
            $sql = "SELECT userID FROM user_follow WHERE userID = :userID AND followed_user_ID = :followed_user_id";
            $sth = $this->db->prepare($sql);
            $sth->execute(array(':userID' => $_SESSION["user_id"], ':followed_user_id' => $user_id));
            $count =  $sth->rowCount();
            if($count>0){
                $user->user_is_followed = TRUE;
            }else{
                $user->user_is_followed = FALSE;
            }
        }
        return $user;
    }
