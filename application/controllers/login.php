...
class Login extends Controller
{
    ...
    /**
     *Allow user to edit their interests
     */
    function editInterests()
    {
        Auth::handleLogin();
        $this->view->render('login/editinterests');
    }
    
    function editInterests_add($interestID=0, $remove=false){
        Auth::handleLogin();
        $login_model = $this->loadModel('Login');
        $json = $login_model->editInterests_add($interestID,$remove);
        if($json){
            echo $json;
        }
    }
    ...
    function _addUserToFollow($userID,$remove=false){
        Auth::handleLogin();
        $login_model = $this->loadModel('Login');
        $json = $login_model->_addUserToFollow($userID,$remove);
        if($json){
            echo $json;
        }
    }
